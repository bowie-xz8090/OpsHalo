/**
 * install opshalo from binary
 * After npm i -g opshalo, running `opshalo` will:
 * 1. Download the appropriate binary for the platform
 * 2. Extract it to the package directory (opshalo/)
 * 3. The launcher (npm/opshalo) then launches the extracted binary
 *
...
// Directory where opshalo package is installed
const packageRoot = resolve(__dirname, '..')
// Directory where the extracted binary will live
const extractDir = join(packageRoot, 'opshalo')
...
function showFinalMessage () {
  console.log('')
  console.log('========================================')
  console.log('opshalo installation complete!')
  console.log('========================================')
  console.log('')
  console.log('For more information, documentation, and updates, please visit:')
  console.log('https://github.com/bowie-xz8090/OpsHalo')
  console.log('')
  console.log('Thank you for using opshalo!')
  console.log('========================================')
  console.log('')
}
...
/**
 * Get the path to the extracted opshalo executable
 */
function getElectermExePath () {
  if (plat === 'win32') {
    return join(extractDir, 'opshalo.exe')
  }
  // Linux and macOS (if extracted)
  return join(extractDir, 'opshalo')
}

/**
 * Check if the opshalo binary has been extracted already
 */
...
async function runLinux (folderName, filePattern) {
  ...
  const target = join(packageRoot, `opshalo-${ver}-${folderName}`)
  ...
  const tmpDir = join(packageRoot, '.opshalo-tmp')
  rm('-rf', tmpDir)
  ...
  if (plat === 'linux') {
    ...
    if (fs.existsSync(chromeSandboxPath)) {
      ...
      console.log('  Otherwise, opshalo will launch with --no-sandbox automatically.')
    }
  }
...
async function runWin (archName) {
  ...
  const target = join(packageRoot, `opshalo-${ver}-win-${archName}`)
  ...
  if (!fs.existsSync(exePath)) {
    throw new Error(`opshalo.exe not found at ${exePath} after extraction. Archive may have an unexpected structure.`)
  }
...
async function runWin7 () {
  ...
  const target = join(packageRoot, `opshalo-${ver}-win7`)

  rm('-rf', [target, extractDir])
  fs.mkdirSync(extractDir, { recursive: true })

  console.log('  Fetching release info...')
  const releaseInfo = await getReleaseInfo(r => /opshalo-\d+\.\d+\.\d+-win7\.tar\.gz$/.test(r.name))
  if (!releaseInfo) {
...
async function runMac (archName) {
  ...
  try {
    ...
    console.log('')
    console.log('  Installation complete! You can now launch opshalo from /Applications')
  } catch (err) {
    console.error('')
...
async function main () {
  console.log('')
  console.log('========================================')
  console.log('opshalo binary installer')
  console.log('========================================')
  ...
  try {
    ...
    console.error('')
    console.error('Please visit https://github.com/bowie-xz8090/OpsHalo for release information.')
    console.error('========================================')
    console.error('')
    process.exit(1)
  }
