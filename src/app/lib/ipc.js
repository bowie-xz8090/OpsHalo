...
async function initAppServer () {
  ...
  if (!globalState.get('agentCapabilitySecret')) {
    globalState.set('agentCapabilitySecret', crypto.randomBytes(32).toString('base64url'))
  }
  globalState.set('langMap', langMap)
  if (!globalState.get('serverInited')) {
    const child = await initServer({
      ...config,
      agentCapabilitySecret: globalState.get('agentCapabilitySecret')
    }, {
      ...process.env,
      appPath,
      sshKeysPath
    ...
    globalState.set('serverChild', child)
  }
