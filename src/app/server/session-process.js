    ...
    const responseHandler = (response) => {
      if (response.id === msg.id) {
        ...
        if (response.error) {
          ...
          error.safeMessage = response.error.safeMessage
          reject(error)
        } else {
...
exports.terminals = function (pid) {
  ...
  return {
    ...
    agentCancelExec: async (invocationId, id) => {
      return sendMsgToChildProcess(pid, {
        id,
        action: 'agent-cancel-exec',
        body: { pid, invocationId }
      })
    },
    resize: (cols, rows, id) => {
