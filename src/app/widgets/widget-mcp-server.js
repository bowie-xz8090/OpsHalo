...
const { assertLegacyMcpGatewayBoundary } = require('../agent/policy/legacy-mcp-gate')
...
class ElectermMCPServer {
  ...
  static get BUILTIN_BLACKLIST () {
    return builtinDenyRules.map(rule => rule.expression)
  }
  ...
  validateCommand (command) {
    ...
    const builtinMatches = matchBuiltinDeny(command)
    if (builtinMatches.length) return { allowed: false, reason: `Command blocked by built-in safety rule: ${builtinMatches.map(item => item.id).join(', ')}` }
    ...
    // Invalid expressions fail closed in whitelist mode and are ignored in a
    // blacklist, matching the unified Policy Engine's stricter-result rule.
    const userWhitelist = (this.config.commandWhitelist || '')
      .split('\n')
      .map(s => s.trim())
      .filter(Boolean)

    const configuredRule = matchUserCommandRules(command, { commandBlacklist: userBlacklist, commandWhitelist: userWhitelist })
    if (configuredRule) {
      return {
        allowed: false,
        reason: configuredRule.code === 'user_command_blacklist' ? `Command blocked by blacklist: ${configuredRule.message}` : `Command not in whitelist: ${configuredRule.message}`
      }
    }
  ...
  sendToRenderer (action, data, timeoutMs = 30000) {
    assertLegacyMcpGatewayBoundary(globalState.get('config') || {}, action, data)
