...
import { safeGetItemJSON } from '../common/safe-local-storage'
import { recoverAiConfigFromHistory } from '../components/ai/ai-config-recovery.mjs'

const AI_CONFIG_HISTORY_KEY = 'ai_config_history'

function recoverSavedAiConfig (store) {
  const recovery = recoverAiConfigFromHistory(
    store.config,
    safeGetItemJSON(AI_CONFIG_HISTORY_KEY, [])
  )
  if (recovery.recovered) {
    store.setConfig(recovery.patch)
    console.info('[ai-config] Recovered protected AI settings after blank configuration fallback.')
  }
  return recovery
}
