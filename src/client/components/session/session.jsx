...
import { normalizeAgentInputMode } from '../terminal/agent-input-mode.mjs'
import './session.styl'
  ...
  handleAiInputModeChange = async (nextMode) => {
    ...
    try {
      if (mode === 'shell') {
        ...
        if (terminal?.isAgentSessionActive?.()) {
          const cancelled = await terminal.handleAgentControl('cancel')
          if (!cancelled) return
        }
      }
      this.editTab({ aiInputMode: mode })
    } finally {
      this.setState({ aiModeSwitching: false })
    }
  }
