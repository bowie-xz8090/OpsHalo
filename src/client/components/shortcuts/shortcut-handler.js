...
import { resolveCtrlCAction } from './ctrl-c-action.mjs'

export { resolveCtrlCAction } from './ctrl-c-action.mjs'
  ...
  Cls.prototype.handleKeyboardEvent = function (event) {
    ...
    if (
      ...
      if (!event.__electermAiCtrlCHandled) {
        event.__electermAiCtrlCHandled = true
        const domSelection = String(window.getSelection?.() || '').trim()
        const action = resolveCtrlCAction({
          hasSelection: !!this.term.hasSelection() || !!domSelection,
          terminalHandoff: this.agentTerminalHandoff === true,
          aiActive: this.isAgentSessionActive?.() || window.store.agentRunning === true || ['pending', 'streaming'].includes(this.state?.smartShellProposal?.status)
        })
        if (action === 'copy_selection') {
          event.preventDefault?.()
          event.stopPropagation?.()
          if (this.term.hasSelection()) this.copySelectionToClipboard?.()
          else document.execCommand?.('copy')
          return false
        }
        if (action === 'send_terminal_sigint') return true
        if (action === 'cancel_ai') {
          event.preventDefault?.()
          event.stopPropagation?.()
          if (this.isAgentSessionActive?.()) {
            this.handleAgentControl?.('cancel', { reason: 'ctrl_c' })
          } else if (window.store.agentRunning && window.store.agentRequestId) {
            window.pre.runGlobalAsync('stopStream', window.store.agentRequestId).catch(() => {})
          } else if (this.state?.smartShellProposal) {
            const requestId = this.state.smartShellProposal.id
            this.smartShellProposalId = ''
            this.setState({
              smartShellProposal: {
                ...this.state.smartShellProposal,
                status: 'cancelled',
                message: '已通过 Ctrl+C 中断 AI 请求。'
              }
            })
            window.pre.runGlobalAsync('stopStream', requestId).catch(() => {})
          }
          return false
        }
      }
