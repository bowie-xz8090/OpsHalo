...
async function callBackendAIchatWithTools (messages, config) {
  const requestId = `legacy_agent_${crypto.randomUUID()}`
  window.store.agentRequestId = requestId
  try {
    return await window.pre.runGlobalAsync(
      'AIchatWithTools',
      messages,
      config.modelAI,
      config.baseURLAI,
    ...
    if (window.store.agentRequestId === requestId) window.store.agentRequestId = ''
  }
}
