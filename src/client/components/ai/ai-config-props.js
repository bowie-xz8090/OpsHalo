...
export function isApiAiConfigMissing (config = {}) {
  return requiredAiConfigsArr.some(key => !String(config[key] || '').trim())
}

export function isAgentAiConfigMissing (config = {}) {
  const backend = config.aiBackendType === 'codex_subscription' ? 'codex_subscription' : 'openai_compatible'
  if (backend === 'codex_subscription') {
    return !String(config.codexProfileId || '').trim()
  }
  return isApiAiConfigMissing(config)
}
